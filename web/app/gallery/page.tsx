...
export default function GalleryPage() {
  ...
  const delSelected = async ()=>{await globalThis.fetch("/api/captures",{method:"DELETE",body:JSON.stringify({ids:[...selected]}),headers:{"Content-Type":"application/json"}});setSelected(new Set());load();setMsg("ลบแล้ว")};
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8">
      <div className="mb-6 flex items-center justify-between">
        ...
        <div className="flex gap-2">{items.length>0&&<><button onClick={selectAll} className="rounded-xl border border-zinc-300 px-3 py-2 text-sm dark:border-zinc-600 dark:text-zinc-300">{selected.size===items.length?"ยกเลิก":"เลือกทั้งหมด"}</button>{selected.size>0&&<button onClick={delSelected} className="rounded-xl bg-red-600 px-3 py-2 text-sm text-white hover:bg-red-700">ลบ ({selected.size})</button>}</>}<button onClick={load} className="rounded-xl border px-3 py-2 text-sm dark:border-zinc-600 dark:text-zinc-300">รีเฟรช</button><a href="/" className="text-sm text-blue-600 hover:underline self-center ml-2">← กลับ</a></div>
      </div>
